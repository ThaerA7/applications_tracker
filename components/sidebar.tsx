  ...
  useEffect(() => {
    ...
    const refreshCounts = async () => {
      ...
      try {
        ...
        safeSet(() => setAppliedCount(items.length));
      } catch (err) {
      ...
      try {
        ...
        safeSet(() => setInterviewsCount(items.length));
      } catch (err) {
      ...
      try {
        ...
        safeSet(() => setWithdrawnCount(items.length));
      } catch (err) {
      ...
      try {
        ...
        safeSet(() => setRejectedCount(items.length));
      } catch (err) {
      ...
      try {
        ...
        safeSet(() => setOffersCount(items.length));
      } catch (err) {
        console.error("Failed to load offers count", err);
        safeSet(() => setOffersCount(0));
      }

      // ✅ Wishlist (Supabase or guest) — THIS is the main fix
      try {
        const { items } = await loadWishlist();
        safeSet(() => setWishlistCount(items.length));
      } catch (err) {
        console.error("Failed to load wishlist count", err);

        // fallback to localStorage if wishlist storage helper fails
        if (typeof window !== "undefined") {
          try {
            const raw = window.localStorage.getItem(WISHLIST_STORAGE_KEY);
            const parsed = raw ? JSON.parse(raw) : null;
            safeSet(() => setWishlistCount(Array.isArray(parsed) ? parsed.length : 0));
          } catch {
      ...
      try {
        ...
        const parsed = raw ? JSON.parse(raw) : null;
        safeSet(() => setNotesCount(Array.isArray(parsed) ? parsed.length : 0));
      } catch (err) {
    ...
    const onStorage = (e: StorageEvent) => {
      // other tabs updates
      if (!e.key) return;
      if (e.key === WISHLIST_STORAGE_KEY || e.key === NOTES_STORAGE_KEY) {
        void refreshCounts();
      }
    };
          ...
          {items.map(({ href, label, icon }) => {
            ...
            return (
              <li key={href}>
                ...
                >
                  <div className={collapsed ? "flex items-center" : "flex items-center gap-3 flex-1"}>
                    <Image
                      src={icon}
                      alt=""
                      width={35}
                  ...
                  {/* Interviews */}
                  {!collapsed && isInterviewsItem && interviewsCount !== null && (
                    <span className="ml-2 inline-flex items-center justify-center rounded-full bg-zinc-800 px-2.5 py-0.5 text-xs font-semibold text-zinc-100">
                      {interviewsCount}
                    </span>
                  )}
                  ...
                  {/* Withdrawn */}
                  {!collapsed && isWithdrawnItem && withdrawnCount !== null && (
                    <span className="ml-2 inline-flex items-center justify-center rounded-full bg-zinc-800 px-2.5 py-0.5 text-xs font-semibold text-zinc-100">
                      {withdrawnCount}
                    </span>
                  )}

                  {/* ✅ Wishlist (now correct for Supabase + guest) */}
