...
import {
  Search, MapPin, ExternalLink, Loader2, ChevronLeft, ChevronRight, Tag,
  Code2, Server, Stethoscope, Hammer, Wrench, GraduationCap, Palette, Truck, ChefHat, Building2, Briefcase, Ruler,
} from 'lucide-react';
...
type JobRow = {
  ...
  logoUrl?: string;
  distanceKm?: number;
  offerType?: string | number; // BA angebotsart OR arbeitszeit (when present)
};
...
function FloatingMenu({
  anchorRef, open, onClose, width = 224, children, align = 'right',
}: {
  anchorRef: React.RefObject<HTMLElement>;
  open: boolean; onClose: () => void; width?: number; children: ReactNode; align?: 'left' | 'right';
}) {
...
function AutoCompleteInput({
  value, onChange, placeholder, ariaLabel, suggestionsSource, icon, name, submitOnPick = true, rightAddon, rightPadClass = 'pr-3',
}: {
  value: string; onChange: (v: string) => void; placeholder: string; ariaLabel: string;
  suggestionsSource: string[]; icon: 'search' | 'location'; name?: string; submitOnPick?: boolean; rightAddon?: ReactNode; rightPadClass?: string;
}) {
  ...
  useEffect(() => { setActive(0); setOpen(items.length > 0 && value.trim().length >= 1); }, [items.length, value]);
  ...
  useLayoutEffect(() => {
    ...
    return () => { events.forEach((ev) => window.removeEventListener(ev, update)); cancelAnimationFrame(raf); };
  }, [open, value]);
    ...
    >
      {items.map((s, idx) => (
        <li
          ...
          onMouseDown={(e) => { e.preventDefault(); onChange(s); setOpen(false); if (submitOnPick) inputRef.current?.form?.requestSubmit(); }}
  ...
  return (
    <div ref={rootRef} className="relative">
      <div className={['pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 z-10', icon === 'search' ? 'text-amber-600' : 'text-orange-600'].join(' ')} aria-hidden="true">
      ...
      <input
        ...
        onKeyDown={(e) => {
          if (open) {
            ...
            if (e.key === 'Enter' && items[active]) { e.preventDefault(); onChange(items[active]); setOpen(false); (inputRef.current?.form as HTMLFormElement | undefined)?.requestSubmit(); return; }
            if (e.key === 'Escape') { setOpen(false); return; }
          }
          if (e.key === 'Enter') (inputRef.current?.form as HTMLFormElement | undefined)?.requestSubmit();
        }}
        className={['h-11 w-full rounded-lg text-sm text-neutral-900 placeholder:text-neutral-500', 'bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/80', 'border border-neutral-200 ring-1 ring-transparent', 'focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-300', 'transition-shadow', 'pl-10', rightPadClass].join(' ')}
      />
      {rightAddon && (
...
function dedupeJobs(list: JobRow[]): JobRow[] {
  const seen = new Set<string>(); const out: JobRow[] = [];
  for (const j of list) { const id = jobIdentity(j); if (id && seen.has(id)) continue; if (id) seen.add(id); out.push(j); }
  return out;
}

export default function OffersPage() {
  const router = useRouter();
  const [q, setQ] = useState(''); const [wo, setWo] = useState('');
  const [distance, setDistance] = useState<string>(''); // km
  const [offerType, setOfferType] = useState<string>(''); // UI-only filter
  const [offerOpen, setOfferOpen] = useState(false); const [distanceOpen, setDistanceOpen] = useState(false);
  const offerBtnRef = useRef<HTMLButtonElement | null>(null); const distanceBtnRef = useRef<HTMLButtonElement | null>(null);
  ...
  const dq = useDebounced(q, 220); const dwo = useDebounced(wo, 220);

  useEffect(() => {
    if (!dq.trim()) { setKwSugs([]); return; }
    ...
    (async () => {
      ...
      } catch { if (!ac.signal.aborted) setKwSugs([]); }
    })();
    return () => ac.abort();
  }, [dq]);

  useEffect(() => {
    if (!dwo.trim()) { setLocSugs([]); return; }
    const ac = new AbortController();
    (async () => {
      try {
      ...
      } catch { if (!ac.signal.aborted) setLocSugs([]); }
    })();
    return () => ac.abort();
  }, [dwo]);
  ...
  async function runSearch(_form?: HTMLFormElement) {
    ...
    setLoadingFirst(true); setAllResults([]); setTotalAvailable(null); setPage(1); pushUrl(1);
    try {
  ...
  const totalPages = (knownTotal != null ? Math.max(1, Math.ceil(knownTotal / PAGE_SIZE)) : Math.max(1, Math.ceil(Math.max(1, effectiveTotal) / PAGE_SIZE))) || 1;
  ...
  const handlePrev = () => { if (!canPrev) return; const next = page - 1; setPage(next); pushUrl(next); };
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (allResults.length >= need) { setPage(next); pushUrl(next); return; }
      ...
      if (!batch || batch.length === 0) { setTotalAvailable((prev) => (Number.isFinite(prev as number) ? (prev as number) : prevLen)); return; }

      const merged = dedupeJobs([...allResults, ...batch]);
      const newLen = merged.length; setAllResults(merged);
      if (newLen === prevLen && !totalKnown) { setTotalAvailable(prevLen); return; }
      if (batch.length < PAGE_SIZE && !totalKnown) { const computed = (next - 1) * PAGE_SIZE + batch.length; setTotalAvailable(computed); }

      setPage(next); pushUrl(next);
    } finally { setPageLoading(false); }
  };

  // auto-refresh on filter change
  useEffect(() => { const ready = q.trim() && wo.trim(); if (!ready) return; runSearch(); /* eslint-disable-next-line */ }, [distance, offerType]);
  ...
  return (
    <section className={['relative rounded-2xl border border-neutral-200/70', 'bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50', 'p-8 shadow-md overflow-hidden'].join(' ')}>
      ...
      <form className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-[1fr,1fr,auto]" onSubmit={(e) => { e.preventDefault(); runSearch(e.currentTarget); }}>
        <div className="relative">
          <AutoCompleteInput
            name="keywords" value={q} onChange={setQ} placeholder="Job title / keywords (e.g. Verkäufer/in)" ariaLabel="Keywords" suggestionsSource={kwSugs} icon="search" rightPadClass="pr-28"
            rightAddon={
              <>
                <button ref={offerBtnRef} type="button" onClick={() => setOfferOpen((v) => !v)} aria-haspopup="menu" aria-expanded={offerOpen} className={btnInside} title="Offer type">
                  <span className="inline-flex items-center gap-1"><Tag className="h-3.5 w-3.5" />{offerType ? OFFER_LABEL(offerType) : 'Offer'}</span>
                </button>
                <FloatingMenu anchorRef={offerBtnRef as unknown as React.RefObject<HTMLElement>} open={offerOpen} onClose={() => setOfferOpen(false)} width={224} align="right">
                  {OFFER_OPTIONS.map((o) => (
                    <button key={o.value || 'any'} type="button" onClick={() => { setOfferType(o.value); setOfferOpen(false); }}
                      className={['block w-full px-3 py-2 text-left text-sm hover:bg-neutral-50', o.value === offerType ? 'bg-amber-50 text-amber-800' : 'text-neutral-800'].join(' ')} role="menuitem">
                      {o.label}
                    </button>
                  ))}
                </FloatingMenu>
        ...
        <div className="relative">
          <AutoCompleteInput
            name="location" value={wo} onChange={setWo} placeholder="Ort / PLZ" ariaLabel="Ort / PLZ" suggestionsSource={locSugs} icon="location" rightPadClass="pr-28"
            rightAddon={
              <>
                <button ref={distanceBtnRef} type="button" onClick={() => setDistanceOpen((v) => !v)} aria-haspopup="menu" aria-expanded={distanceOpen} className={btnInside} title="Distance">
                  <span className="inline-flex items-center gap-1"><Ruler className="h-3.5 w-3.5" />{distance ? `${distance} km` : 'Distance'}</span>
                </button>
                <FloatingMenu anchorRef={distanceBtnRef as unknown as React.RefObject<HTMLElement>} open={distanceOpen} onClose={() => setDistanceOpen(false)} width={176} align="right">
                  <button type="button" className={['block w-full px-3 py-2 text-left text-sm hover:bg-neutral-50', distance === '' ? 'bg-amber-50 text-amber-800' : 'text-neutral-800'].join(' ')}
                    onClick={() => { setDistance(''); setDistanceOpen(false); }} role="menuitem">Any distance</button>
                  {DISTANCES.map((d) => (
                    <button key={d} type="button" onClick={() => { setDistance(d); setDistanceOpen(false); }}
                      className={['block w-full px-3 py-2 text-left text-sm hover:bg-neutral-50', distance === d ? 'bg-amber-50 text-amber-800' : 'text-neutral-800'].join(' ')} role="menuitem">
                      {d} km
                    </button>
                  ))}
                </FloatingMenu>
        ...
        <div className="flex items-stretch">
          <button
            ...
            className={[
              ...
              submitDisabled ? 'cursor-not-allowed border-neutral-200 bg-neutral-100 text-neutral-400' : 'border-neutral-200 bg-amber-500/90 text-white hover:bg-amber-500',
              'h-11 w-full sm:w-auto', 'focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-amber-300',
            ].join(' ')}
            aria-label="Search" title={submitDisabled ? 'Fill title and location' : 'Search'}
          >
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-3 text-sm text-neutral-700">
        <div>
          {Number.isFinite(totalAvailable as number)
            ? `${(totalAvailable as number).toLocaleString()} offers found${q || wo ? ` for ${q || ''}${q && wo ? ' in ' : ''}${wo || ''}` : ''}`
            : allResults.length > 0 ? `${allResults.length.toLocaleString()} offers loaded` : ''}
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3" aria-live="polite">
        ...
        {!loadingFirst && !pageLoading && displayed.map((job, i) => {
          ...
          return (
            ...
            >
              <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-neutral-200 bg-white ring-1 ring-white/60">
                <div className="absolute left-1 top-1 rounded-md bg-neutral-900/80 px-1.5 py-0.5 text-[11px] font-semibold text-white">{idx}</div>
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-base font-semibold text-neutral-900">{job.employer || 'Unbekannter Arbeitgeber'}</h2>
        ...
        {!loadingFirst && (allResults.length > 0 || page > 1) && (
          <div className="mt-3 flex items-center justify-between">
            <button
              type="button" onClick={handlePrev} disabled={!canPrev} aria-label="Previous page"
              className={[
                'inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm font-medium shadow-sm',
                canPrev ? 'border-neutral-200 bg-white/80 text-neutral-800 hover:bg-white' : 'cursor-not-allowed border-neutral-200 bg-neutral-100 text-neutral-400',
            ...
            <button
              type="button" onClick={handleNext} disabled={!canNext} aria-label="Next page"
              className={[
                'inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm font-medium shadow-sm',
                canNext ? 'border-neutral-200 bg-white/80 text-neutral-800 hover:bg-white' : 'cursor-not-allowed border-neutral-200 bg-neutral-100 text-neutral-400',
                'focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-amber-300',
              ].join(' ')}
            >
